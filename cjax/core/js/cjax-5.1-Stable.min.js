/* CJAX 5.1-Stable */
function CJAX_FRAMEWORK(){this.name='cjax';this.version='5.1-Stable';this.debug=false;var CJAX_CACHE=[];this.DOMContentLoaded=false;this.loaded=[];this.handlers={handlerFileupload:function(form,url,funcRequestCallback){},handlerRequestStatus:function(url,request_status){},handlerFormRequest:function(url,serial,args){},handlerConfirm:function(question,callback){}};var _FUNCTION;var __base__;var __root__;this.uri;this.f;this.base;this.$plugin;this.plugins={};this.pBase;this.vars=[];this.waitingFor={};this.preloadTime=0;this.found_extra_time=0;this.message_id=0;this.commands={};this.callback_success={};this.callback_error=null;this.callback_cache={};this.deferredActions={};this.left_delimeter="<";this.right_delimeter=">";this.split_delimiter="|";this.default_timeout=3;this.messages=[];this.is_loading=true;this.clicked;this.IS_POST=false;this.ie;this.cache_calls=[];this.chache_templates=[];this.dir;this.files=false;this.styles=[];this.current_element=[];this.timer=0;this.funtion_timer=true;this.defaultMessages={invalid:'Invalid Input!',success:'Success!',error:'Error!'};this.ajaxSettings=this.ajaxDefaultSettings={dataType:'text/html',cache:false,process:true,ajaxVars:{},success:function(){},complete:function(){},error:function(){}};var FLAG_WAIT=1;var FLAG_NO_WAIT=2;var FLAG_TIMEOUT_EXPAND=false;var HELPER_STYLE;this.Execfn=function(fn,element,data)
{_raw_fn=fn;switch(fn){case'replace':if(data.b[0]=='#'&&!/[^a-zA-Z0-9_]/.test(data.b.replace(/^#/,''))){data.b=data.b.replace(/^\#/,'');child=CJAX.$(data.b);if(!child){console.log('Element#1',data.b,'could not be found.');return;}
element.parentNode.replaceChild(child,element);}else{data.replace=true;return CJAX.Execfn('append',element,data);}
break;case'before':case'prepend':if(data.b[0]=='#'){data.b=data.b.replace(/^\#/,'');child=CJAX.$(data.b);if(!child){console.log('Element#1',data.b,'could not be found.');return;}
element.parentNode.insertBefore(child,element);}else{CJAX.lib.fnCall(element,{prepend:data.b});}
...
if(data.c){if(element.lastChild){element.insertBefore(child,element.lastChild.nextSibling);}else{element.appendChild(child);}}else{element.insertBefore(child,element.firstChild);}}else{CJAX.lib.fnCall(element,{insert:data.b},data);}
...
break;case'hide':element.style.display='none';break;case'show':element.style.display='block';break;default:CJAX.lib.fnCall(element,{_raw_fn:data.b});}};this._fn=function(buffer)
{fn=_raw_fn=CJAX.xml('fn',buffer);var prop;data=CJAX.util.json(CJAX.decode(CJAX.xml('fn_data',buffer)));switch(fn){case'document':if(typeof document[data.a]=='string'){data.b=CJAX._pharseValues(data.b);return document[data.a]=data.b;}
break;}
...
try{return CJAX.Execfn(_raw_fn,element,data);}catch(e){alert("Function "+_raw_fn+" generated an error: "+e);return;}}else if(type=='direct'){str=_raw_fn.toLowerCase().replace(/\b([a-z])/gi,function(c){return c.toUpperCase();});if(CJAX.handlers['_handler'+str]&&CJAX.lib.isFn(CJAX.handlers['_handler'+str])){if(CJAX.handlers['_handler'+str](data.a,data.b,data,buffer)){return true;}}
...
var _parse;if(typeof JSON!='undefined'&&!CJAX.ie){_parse=JSON.parse;}
if(!tag){var tag='json';}
if(buff=CJAX.decode(CJAX.xml(tag,buffer))){try{if(_parse){json=_parse(buff);}else{json=eval("("+buff+")");}}catch(e){if(CJAX.ie){alert(e.message);}
...
var script;var name='cjax-5.1-Stable.js';var src;script=CJAX.$('cjax_lib');if(script){src=script.src;var f=src.replace(/cjax-.+$/,'');CJAX.uri=script.src;}else{var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){script=scripts[i];src=script.src;if(CJAX.util.get.basename(src).indexOf('cjax-')!=-1){var f=src.replace(/cjax-.+$/,'');CJAX.uri=src;return f;}}}
...
element=element[prop](getter);if(typeof callback=='function'){callback(element);}}else{CJAX.lib.fnCall(element,getter);}},fnCall:function(element,setting){if(CJAX.lib.isFn(element[setting])){element[setting](data.b);}else{if(typeof setting=='object'){for(x in setting){CJAX.lib.fns(element,x,setting[x],setting,data);}}else{element[setting]=callback;}}},fns:function(element,fn,value,obj,data){switch(fn){case'prepend':element['outerHTML']=value+element['outerHTML'];break;case'append':if(data.replace){element['outerHTML']=value;}else{element['outerHTML']=element['outerHTML']+value;}
break;case'insert':if(data.c){element['innerHTML']=element['innerHTML']+value;}else{element['innerHTML']=value+element['innerHTML'];}
...
var head=CJAX.elem_docs('body')[0];head.appendChild(s);return s;}};}();this.process_all=function(actions,preload,debug,is_loading)
...
if(CJAX[SUBFIX]){try{if(seconds){buffer=buffer.replace(/[\n\r]/gm,'\\n');setTimeout(PREFIX+CJAX.method+'("'+buffer+'")',seconds*1000);}else{fn=CJAX[SUBFIX];fn(buffer);}}catch(_e){alert('#process unabled to load function#1: '+CJAX.method+'();  '+_e.message);}
...
catch(_e){try{xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}
catch(_e){xmlhttp=false;}}
...
var data=CJAX.xml('data',buffer);if(data){data=CJAX.decode(data);element.innerHTML=data;}else{element.innerHTML='';}};this.location=function(buffer){var destination=CJAX.xml('url',buffer);window.location=destination;};this._serialize=function(form,include_files){if(typeof include_files=='undefined'){var include_files=true;}else{CJAX.files=false;}
form=CJAX.is_element(form);var url='';var elem_value='';var is_my_radio=new Array(10);var splitter;var assign='=';var elems=form.elements?form.elements:elems;var form_len=elems.length;for(var n=0;n<form_len;n++){splitter='&';elem=elems[n];elem_id=elem.id;elem_type=elem.type;elem_name=elem.name;elem_value=elem.value;elem_len=elem.length;if(!elem_type)continue;if(elem_type=='file'&&include_files){CJAX.files=true;}
...
switch(elem_type){case'submit':if(elem_name.toLowerCase()=='submit'){elem.name='btnSubmit';}
continue;break;case'ckeditor':elem_value=(eval("CKEDITOR.instances."+elem_id+".getData()"));break;case'checkbox':elem_value=((elem.checked)?1:'');if(!elem_value){continue;}
...
if(!form_id){var element_id=CJAX.xml('event_element_id',buffer);if(element_id){form=CJAX.$(element_id).form;}else{if(typeof CJAX.clicked=='undefined'){console.log("Form not found");return;}
form=CJAX.clicked.form;}}else{form=CJAX.is_element(form_id);if(!form){form=document.forms[form];if(!form){form=CJAX.$(form);}}}
...
CJAX.files=CJAX._handleUploads(form,url,handlerFormRequest);}else{if(url){handlerFormRequest();CJAX.loading();}}};this.refresh=function()
...
var full_url=url.replace(/\&+$/,'')+'&'+serial+'&'+args;try{CJAX.HTTP_REQUEST_INSTANCE.send(((is_post)?full_url:null));}catch(e){console.log(e);};};this.setHandler=function(name,callback_handler)
...
iframe=CJAX.create.frame('frame_upload');iframe.style.display='none';iframe.width='400';iframe.height='200';form.appendChild(iframe);with(form){method='POST';action=CJAX.f+'?controller=uploader&function=upload&cjax_iframe=1';enctype="multipart/form-data";target=iframe.name;}
form.submit();CJAX.lib.loadCallback(iframe,function(){response=iframe.contentWindow.document.body.innerHTML;CJAX.process_all(response);if(url){$callback(false);}});return iframe;};this.$=function(element_id,v){if(!element_id){return;}
if(typeof element_id=='object'){return element_id;}
if(typeof v=='undefined'){var v=false;}
if(element_id=='body'){return CJAX.elem_docs('body')[0];}
if(element_id=='head'){return CJAX.elem_docs('head')[0];}
element_id=element_id.replace(/^\#/,'');if(/[^a-zA-Z0-9_]/.test(element_id)){console.log('Invalid Element ID:',element_id);return;}
return CJAX.is_element(element_id,v);};this.is_element=function(id_obj,verbose){var type=(typeof id_obj);if(typeof verbose=='undefined'&&CJAX.debug){verbose=true;}
...
try{CJAX.HTTP_REQUEST_INSTANCE.send(((CJAX.IS_POST)?full_url:null));}catch(e){console.log(e);};return response;};this._handleRequestHeaders=function(url,args)
...
CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With','CJAX FRAMEW0RK 5.1-Stable');return full_url;};this._handlerRequestStatus=function(url,status,container)
...
if(CJAX.ajaxSettings.task&&CJAX.lib.isFn(CJAX.ajaxSettings.task)){CJAX.ajaxSettings.task();CJAX.ajaxSettings.task=null;}
CJAX.ajaxSettings=CJAX.ajaxDefaultSettings;return response;};this._message=function(buffer){var message_id;var time;var seconds;var options={};if(typeof buffer=='object'){if(buffer.message_id){message_id=buffer.message_id;}else{message_id='cjax_message';}
