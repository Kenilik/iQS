...
function CJAX_FRAMEWORK() {
	...
	*/
	this.select		=			function( buffer ) {
		...
		if(element.type=='select-one') {
			...
			function make_input(element) {
				...
				var obj = CJAX.create.input(element_id+'_input');
				obj.type = 'text';
				obj.name = element.name;
				obj.className = element.className;
	...
	this._overLayContent		=		function( buffer )
	{
		...
		message_options = {
			...
			success: function() {
				CJAX.lib.overlayCallback(content,callback);
			}
		};
		for(x in options) {
		...
		message_options = {
			...
			success: function() {
				...
				CJAX.get(url, function(response) {
					CJAX.lib.overlayCallback(response,callback);
				});
			}
		};
		for(x in options) {
	...
	this.lib	=	function(element) {
		...
		return {
			overlayCallback: function(response,callback) {
				if(callback) {
			...
			event : function(element,event,method,cache_id){
				...
				if(f =='function' ||CJAX.lib.isFn(method)) {
					if(typeof method=='function') {
						return CJAX._addEvent(element ,event , method, cache_id);
					} else {
						return CJAX._addEvent(element ,event ,eval(method), cache_id);
					}
				}
				...
				return CJAX._addEvent(element, event, function() {_fn(_x[cache_id]);}, cache_id);
			}
	...
	*/
			...
			for(_id in actions) {
				...
				if(CJAX.xml('is_plugin', buffer)) {
					...
					if(waitfor = CJAX.xml('waitfor', plugin_buffer)) {
						if(CJAX.debug) {
							console.info(plugin_method, 'waiting for', waitfor);
						}
					...
					_p = window[plugin_method];

					callbacks = CJAX.xml('callback', plugin_buffer);
					
					if(CJAX.debug && callbacks) {
						console.info('Plugin Buffer',plugin_buffer);
						console.info("Callbacks found for", plugin_method, callbacks);
					}
					if(_p) {
						if(plugin = CJAX.plugins[plugin_method] || CJAX.lib.isFn(_p)) {
							if(CJAX.lib.isFn(plugin)) {
								plugin = CJAX.extend(plugin, plugin_method, plugin_buffer,callbacks);
								
							} else if(CJAX.lib.isFn(_p)) {
								plugin = CJAX.extend(_p, plugin_method, plugin_buffer,callbacks);
						...
						CJAX.lib.loadCallback(CJAX.loaded[f], function(){
							...
							if(CJAX.lib.isFn(window[plugin_method])) {
								...
								plugin = CJAX.extend(window[plugin_method], plugin_method, plugin_buffer, callbacks);
							} else {
								console.log('option ??');
							}
						},null, plugin_method);
	...
	this.extend			=	function(plugin_fn, plugin_name, buffer, callbacks)
	{
		...
		if(buffer) {
			...
			for(x in params) {
				letter =  params[x];
				params[letter] = CJAX.decode(CJAX.xml(letter,buffer));
	
				if(json = CJAX.util.json(params[letter])) {
					for(prop in json) {
						_prop = json[prop];
						json[prop] = CJAX.lib.pharseFunction(json[prop]);
					}
					params[letter] = json;
				}
			}
		 ...
		 _plugin	=	function(buffer) {
			 func = {
				...
				loading : CJAX.loading,
			 	...
			 	importFile: function(_file) {
					if(/^https?/.test(_file)) {
						return CJAX.script.load(_file);
					}
					return CJAX.script.load(base+_file);
				},
				fn: fn
			 };
			 for(x in extra) {
	...
	*/	
	...
	this._wait		=		function( buffer ) {
		...
		var count = parseInt(CJAX.xml('timeout',buffer));
	...
	*/
	...
	this._form		=		function( buffer ) {
		...
		if(CJAX.handlers._handlerFileupload && CJAX.lib.isFn(CJAX.handlers._handlerFileupload)) {
			CJAX.files = CJAX.handlers._handlerFileupload(form, url, handlerFormRequest);
		} else if(CJAX.files) {
			if(CJAX.debug) {
	...
	this._handlerFormRequest		= 	function(url, serial, args) 
	{
		...
		var full_url = url.replace(/\&+$/,'')+'&'+serial+'&'+args;
	...
	{
		CJAX.handlers['_'+name] = callback_handler;
	};
	...
	*/
	...
	this._handleRequestHeaders		=		function(url,args)
	{
		...
		CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With', 'CJAX FRAMEW0RK 5.1-Stable');
