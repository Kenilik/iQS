...
class CoreEvents extends cjaxFormat {
	...
	public function plugin($plugin_name)
	{
		if($this->isPlugin($plugin_name)) {
			if($plugin = plugin::getPluginInstance($plugin_name)) {
				return $plugin;
			}
		}
	}
...
if (document.addEventListener) {
	...
	public function init($min = true)
	{
		if($min && substr($min, 0, 2)=='..') {
			if(strpos($min,'.js')!==false) {
				...
				$this->_file = "cjax-5.1-Stable.min.js";
			}
		} else {
			$this->_file = "cjax-5.1-Stable.min.js";
		}
	...
	function remote($url, $re_retry = false)
	{
		$ajax = CJAX::getInstance();
		...
		if(!$re_retry && function_exists('curl_init')) {
			$ch = curl_init($url);
	...
	function isAjaxRequest()
	{
		...
		if(!empty($headers)) {
			if($headers['X-Requested-With']=='CJAX FRAMEW0RK 5.1-Stable' || $headers['X-Requested-With']=='XMLHttpRequest') {
				return true;
			}
		}
