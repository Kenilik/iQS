...
/**
 * Originally ajax.php, 4 CI is: ajaxfw.php.
 * 
 * $Rev: 1507 $
 **/
class ajax  {
	...
	function ajax($controller)
	{
		...
		if($controller=='_crossdomain') {
			return call_user_func_array(array($ajax, 'crossdomain'), $args);
		}
		
		$is_file = $this->_files($controller);
		...
		$this->_response( call_user_func_array(array($requestObject, $function), $args) );
	}
	...
	protected function _files($controller)
	{
		$ajax = ajax();
		if($plugin = $ajax->plugin($controller)) {
			$files[] = "{$plugin->controllers_dir}/{$controller}.php";
		}
